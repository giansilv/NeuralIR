...
function calculateAndSaveMeasures(start, fin)
  ...
  for i = start : fin
     ...
     %import the pool
    load(COLLECTIONS.path.pool(collectionID));
     
     %load the runSet
        load(COLLECTIONS.path.runSet(collectionID));   
     ...
     for m = 1 : length(measures)
         ...
         if(strcmp(measures{m}, 'twist'))
             fprintf('Calculating: %s \n\n', measures{m});
              ...
              save(COLLECTIONS.path.measureSet(collectionID, measures{m}), COLLECTIONS.identifiers.measureId(collectionID, measures{m}));
         else
