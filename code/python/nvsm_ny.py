...
flags.DEFINE_integer("ngram_size", 16,
                     "The number of words to compose ngram used for the text matching function.")
...
flags.DEFINE_string("corpus", "ny", "corpus to use.")
flags.DEFINE_string("model_name", "nvsm_ny", "Model name.")
FLAGS = flags.FLAGS
...
class NVSM(object):
    ...
    def __init__(self, _word_vocab_size, _corpus_size, _options):
        ...
        with tf.device('/cpu:0'):  # store doc_embs within CPU due to their size
...
def main(_):
    ...
    opts = Options()
    # set folders
    data_folder_name = 'corpus/' + opts.corpus + '/' + opts.corpus
    model_folder_name = 'corpus/' + opts.corpus + '/models'
    quick_folder_name = 'corpus/' + opts.corpus + '/quick_loads'
    query_folder_name = 'corpus/' + opts.corpus + '/queries'
    qrels_folder_name = 'corpus/' + opts.corpus + '/qrels'
    rankings_folder_name = 'corpus/' + opts.corpus + '/rankings'
    # set stoplist path
    stoplist_path = './indri_stopwords.txt'
    # load utils functions - set seed value
    utils = Utils(opts.seed)

    if not os.path.exists(rankings_folder_name + '/' + opts.model_name):
        os.makedirs(rankings_folder_name + '/' + opts.model_name)
    if not os.path.exists(model_folder_name + '/' + opts.model_name):
        os.makedirs(model_folder_name + '/' + opts.model_name)
    if not os.path.exists(query_folder_name) or not os.path.exists(qrels_folder_name):
    ...
    if not os.path.exists(quick_folder_name + '/proc_corpus'):  # corpus needs to be processed
        print('processing corpus')
        proc_corpus, doc_labels = utils.process_corpus(data_folder_name, stoplist_path, remove_digits=True)
        # store processed corpus as JSON object (too big for pickle)
        with open(quick_folder_name + '/proc_corpus', 'w') as cf:
            json.dump(proc_corpus, cf)
        # store doc labels as pickle object
        with open(quick_folder_name + '/doc_labels', 'wb') as lf:
            pickle.dump(doc_labels, lf)
    else:  # corpus exists
        print('loading processed corpus')
        with open(quick_folder_name + '/proc_corpus', 'r') as cf:
            # load processed corpus
            proc_corpus = json.load(cf)
        with open(quick_folder_name + '/doc_labels', 'rb') as lf:
            # load doc labels
            doc_labels = pickle.load(lf)
    ...
    #utils.corpus_statistics(proc_corpus)
    ...
    if not os.path.exists(quick_folder_name + '/term_dictionary'):  # term dictionary needs to be processed
        ...
        utils.build_term_dictionary(proc_corpus)
    ...
    if not os.path.exists(quick_folder_name + '/enc_corpus'):  # corpus needs to be encoded
        print('encoding corpus')
        enc_corpus = utils.corpus2idx(proc_corpus, opts.oov)
        # store encoded corpus as JSON object (too big for pickle)
        with open(quick_folder_name + '/enc_corpus', 'w') as cf:
            json.dump(enc_corpus, cf)
    else:  # corpus exists
        print('loading corpus')
        with open(quick_folder_name + '/enc_corpus', 'r') as cf:
            # load encoded corpus
            enc_corpus = json.load(cf)   

    # save memory
    del proc_corpus
    # compute corpus statistics
    #print('compute encoded corpus statistics')
    #utils.corpus_statistics(enc_corpus)

    # load queries
    print('loading queries')
    qdict = utils.read_trec_queries(query_folder_name + '/ny.topics.txt')

    # get list of allowed document indexes
    allowed_docs = utils.get_allowed_docs(enc_corpus, opts.ngram_size)
    # start session
    with tf.Graph().as_default(), tf.Session() as sess:
        # declare model parameters
        num_batches = utils.compute_num_batches(enc_corpus, opts.batch_size, opts.ngram_size)
        ...
        # setup the model
        model = NVSM(len(utils.term_dict), len(enc_corpus), opts)
        ...
        for epoch in range(opts.epochs):
            ...
            for i in range(num_batches):
                # generate batch data
                batch_data = utils.generate_batch_data(enc_corpus, allowed_docs, opts.batch_size, opts.ngram_size, opts.neg_samples)
                # feed feed_dict
            ...
            for qid, qtext in qdict.items():  # TODO: make field choice as a flag param
                ...
                proj_query = utils.prepare_query(qid, qtext['title'], word_embs, proj_weights, opts.oov)
            ...
            map_value = utils.evaluate_rankings(rankings_folder_name + '/' + opts.model_name, opts.model_name + str(epoch + 1), qrels_folder_name, 'ny')
    ...
    with tf.Session() as sess:
        ...
        saver = tf.train.import_meta_graph('./corpus/' + opts.corpus + '/models/' + opts.model_name + '/' + opts.model_name + str(best_epoch) + '.ckpt.meta')
        saver.restore(sess, './corpus/' + opts.corpus + '/models/' + opts.model_name + '/' + opts.model_name + str(best_epoch) + '.ckpt')
        ...
        utils.evaluate_rankings(rankings_folder_name + '/' + opts.model_name, opts.model_name + 'test_epoch' + str(best_epoch), qrels_folder_name, 'ny')
