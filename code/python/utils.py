...
class Utils(object):
    ...
    def associate_terms2cuis(self, pos_terms, concepts_lists):
        """return list of (term, [cui1, cui2, ...]) pairs given terms' positions and candidate concepts lists"""
        for concepts in concepts_lists:
            # store concept positional information - taken from first candidate concept
            start = concepts[0]['start']
            # check if start matches with pos_terms' key
            if start in pos_terms:  # update ["__NULL__"] with term's list of candidate cuis
                pos_terms[start][1] = [concept['cui'] for concept in concepts]
        # return pos_terms' values only ([term, [cui1, cui2, ...]] pairs)
        return list(pos_terms.values())
