...
flags.DEFINE_string("corpus", "wsj", "corpus to use.")
flags.DEFINE_string("model_name", "nvsm_wsj", "Model name.")
FLAGS = flags.FLAGS
...
def main(_):
    ...
    index_folder_name = 'corpus/' + opts.corpus + '/index'
    ...
    if 'ohsu' in opts.corpus:
        qdict = utils.read_ohsu_queries(query_folder_name + '/query.ohsu.all')  # @smarchesin TODO: make it a choice with flags
    elif 'clef' in opts.corpus:
        qdict = utils.read_clef_queries(query_folder_name + '/AH_FA_2008-2009_topics.xml')  # @smarchesin TODO: make it a choice with flags
    else:
        qdict = utils.read_trec_queries(query_folder_name + '/topics.51-200.txt')  # @smarchesin TODO: make it a choice with flags
    ...
    if not os.path.exists(qrels_folder_name + '/val_qrels.qrel') or not os.path.exists(qrels_folder_name + '/test_qrels.qrel'): 
        # read qrels
        with open(qrels_folder_name + '/wsj_51-200.qrel', 'r') as qrels_file:
            qrels = qrels_file.readlines()
        ...
        # write qrels 
        with open('./corpus/' + opts.corpus + '/qrels/val_qrels.qrel', 'w') as out_val, open('./corpus/' + opts.corpus + '/qrels/test_qrels.qrel', 'w') as out_test:
            # validation qrels
